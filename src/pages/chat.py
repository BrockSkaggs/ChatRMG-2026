...
@callback(
    ...
    State(ChatSettingsAIO.ids.conversation_div("settings-aio"), "children"),
    prevent_initial_call=True,
)
def add_chat_card(n_clicks, user_prompt, chat_history, 
                  raw_chat_history, conversation_id, conversation_div
    ):
    ...
    patch_div = inspect_conversation_div(conversation_id, conversation_div)
...
def update_context(
    ...
    formatted_context_str = user_prompt
...
def streaming_chat():
    ...
    model_select = request.json["model_select"]
    conversation_id = request.json["conversation_id"]

    chat_completion_prompt = []
    if chat_history is None or len(chat_history['chat_history']) == 1:
        system_prompt = {"role": "system", "content": load_system_prompt()}        
        chat_completion_prompt.insert(0, system_prompt)
        chat_completion_prompt.append({"role": "user", "content": user_prompt})
    else:
        chat_completion_prompt.append({"role": "user", "content": context_str})

    logger.debug(f"chat Prompt: {chat_completion_prompt}")

    #Original
    def response_stream():
        for line in stream_send_messages(chat_completion_prompt, model_select, conversation_id):
            if type(line) == tuple:
                ai_chunk = line[0]
                if ai_chunk.content != '':
                    if ai_chunk.type == 'tool':
                        yield f"\n\n Tool Result: {ai_chunk.content}\n\n"
                    else:
                        yield ai_chunk.content
                if len(ai_chunk.content_blocks) > 0 and ai_chunk.content_blocks[0]['type'] == 'reasoning':
                    yield ai_chunk.content_blocks[0]['reasoning']
                else:
                    pass
            else:
                yield line.content
...
def format_chat_history(
    ...
    card_children = [dcc.Markdown(last_generated_response_md)]
...
@callback(
    Output("chat-history", 'children', allow_duplicate=True),
    Output('conversation-id', 'data', allow_duplicate=True),
    Input(ConversationTagAIO.ids.conversation_btn(ALL), 'n_clicks'),
    State(ChatSettingsAIO.ids.model_select("settings-aio"), "value"),
    prevent_initial_call=True
)
def switch_chat_history(clicks, model: str):
    if clicks is None or len(clicks) == 0 or all(c is None for c in clicks):
        raise dash.exceptions.PreventUpdate
    conversation_id = ctx.triggered_id['aio_id']
    print('Conversation ID to recover: ', conversation_id)
    messages = recover_chat_history(model, conversation_id)
    if messages is None:
        raise dash.exceptions.PreventUpdate
...
@callback(    
    Output({"type": "thumbs-up-button", "index": MATCH}, "gradient", allow_duplicate=True),
    Output({"type": "thumbs-down-button", "index": MATCH}, "gradient",allow_duplicate=True),    
